...
$tables = @{
  AccessApi = [string]$config.access_api_table_id
  Persona = [string]$config.persona_table_id
  XyqContent = [string]$config.content_table_id
  RelayContent = [string]$config.relay_content_table_id
  LtxContent = [string]$config.ltx_content_table_id
  PlatformPublish = [string]$config.platform_publish_table_id
}

foreach ($requiredTable in @('AccessApi', 'Persona', 'XyqContent')) {
  if (-not $tables[$requiredTable]) {
    throw "config.json 缺少必需表配置：$requiredTable"
  }
}
...
if ($tables.RelayContent) {
  $linkDefinitions += ,@($tables.RelayContent, (LinkField '文本/分析模型' $tables.AccessApi '生成提示词或分析参考素材的模型'))
}
if ($tables.LtxContent) {
  $linkDefinitions += ,@($tables.LtxContent, (LinkField '分镜分析模型' $tables.AccessApi '分析参考视频并生成分镜提示词的模型'))
  $linkDefinitions += ,@($tables.LtxContent, (LinkField '视频生成模型' $tables.AccessApi '实际生成LTX视频的模型'))
}
if ($tables.PlatformPublish) {
  $linkDefinitions += ,@($tables.PlatformPublish, (LinkField '发布文案模型' $tables.AccessApi '生成标题、文案和标签的模型'))
}
