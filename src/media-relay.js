...
function createMediaRequestHandler({ storageDir = DEFAULT_STORAGE_DIR, identity = null } = {}) {
  return (request, response) => {
    ...
    if (pathname === '/identity' && typeof identity === 'function') {
      response.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
...
function createRelayMediaStore({
  ...
  return {
    async upload(filePath, { recordId, role }) {
      ...
      let state = readRelayState(statePath);
      const checkHealth = async (activeState) => {
        let healthError;
        for (let attempt = 1; attempt <= healthAttempts; attempt += 1) {
          try {
            const response = await fetchImpl(`${activeState.baseUrl}/health`, {
      ...
      let healthError = await checkHealth(state);
      if (healthError && typeof recover === 'function') {
        try {
          await recover({ statePath, storageDir, state, error: healthError });
          state = readRelayState(statePath);
          healthError = await checkHealth(state);
        } catch (recoveryError) {
          healthError = recoveryError;
        }
      }
      if (healthError) {
