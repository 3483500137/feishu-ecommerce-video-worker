...
const { recoverMediaRelay } = require('./media-relay-recovery');
const {
...
function createWorkerRelayMediaStore() {
  return createRelayMediaStore({ recover: recoverMediaRelay });
}
...
function runLark(args) {
  const stdout = run(process.execPath, [LARK_CLI, ...args, '--format', 'json'], { timeoutMs: 60_000 });
...
async function submitRelaySegmentedVideo({ row, recordId, tableId, access, reference, workDir }) {
  ...
  const mediaStore = createWorkerRelayMediaStore();
  const descriptor = {
    mode: 'segmented',
    taskIds: [],
    assetKeys: [],
...
function buildPersonaImageRequest({
  persona,
  userRequirement = '',
  referenceFile = '',
  ...
  return {
    ...
    assetIds: useReferenceFace ? [uploadAsset(referenceFile)] : [],
  };
}
...
function uploadXyqAsset(filePath, route = {}) {
  const script = path.join(CONFIG.xyq_skill_scripts, 'upload_file.py');
  const env = { ...process.env };
  if (route.runtime) {
    const { secret } = route.runtime.getContext({
    ...
    env.XYQ_ACCESS_KEY = secret;
  }
  const stdout = run('python', [script, filePath], { env });
...
async function processPersona(row) {
  ...
  try {
    ...
    const imageRequest = buildPersonaImageRequest({
      persona,
  ...
  try {
    ...
    if (action === 'poll-video') {
      ...
      if (descriptor.mode === 'segmented') {
        ...
        const finalFieldId = CONFIG.relay_final_video_attachment_field_id || 'fld7uFHNur';
...
async function processLtxContent(row) {
  ...
  try {
    ...
    mediaStore = createWorkerRelayMediaStore();
