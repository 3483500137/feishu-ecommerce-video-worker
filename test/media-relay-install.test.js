...
test('media relay forces Cloudflare Quick Tunnels through HTTP/2 and monitors stale addresses', () => {
  const server = fs.readFileSync(
    path.resolve(__dirname, '..', 'src', 'media-relay-server.js'),
    'utf8',
  );
  assert.match(server, /'--protocol', 'http2'/);
  assert.match(server, /连续健康检查失败/);
});
