...
test('relay media store recovers a stale Cloudflare address before publishing media', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'ltx-relay-recovery-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  ...
  const store = createRelayMediaStore({
    ...
    fetchImpl: async (url) => {
      healthChecks.push(url);
      return { ok: url.includes('fresh-edge') };
    },
    recover: async () => {
      recoveryCalls += 1;
  ...
  const uploaded = await store.upload(source, { recordId: 'rec001', role: 'reference' });

  assert.equal(recoveryCalls, 1);
  assert.deepEqual(healthChecks, [
    'https://stale-edge.trycloudflare.com/health',
    'https://fresh-edge.trycloudflare.com/health',
  ]);
  assert.match(uploaded.url, /^https:\/\/fresh-edge\.trycloudflare\.com\/media\//);
  assert.equal(fs.existsSync(path.join(root, 'media', uploaded.assetKey)), true);
});
...
test('relay identity endpoint is only available when the server provides its own PID', async (t) => {
  const server = http.createServer(createMediaRequestHandler({
    ...
    identity: () => ({ relayPid: 12345 }),
  }));
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));

  const identity = await fetch(`http://127.0.0.1:${server.address().port}/identity`);
  assert.equal(identity.status, 200);
  assert.deepEqual(await identity.json(), { relayPid: 12345 });
});
